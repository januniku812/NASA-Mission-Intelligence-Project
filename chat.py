...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler('chat_interface.log'),
...
import ragas_evaluator # importing ragas evaluator will result in third-party framework import error and streamlit/environment may preload ragas causing same error so mock injection must be at the top of the frontend triggering file
import rag_client
import llm_client
...
def initialize_rag_system(chroma_dir: str, collection_name: str):
    ...
    try:
       return rag_client.initialize_rag_system(chroma_dir, collection_name), True, ""
...
def format_context(documents: List[str], metadatas: List[Dict], scores: List[float]) -> str:
    ...
    return rag_client.format_context(documents, metadatas, scores)
...
def evaluate_response_quality(question: str, answer: str, contexts: List[str], evaluator_llm: str, evaluator_llm_embedding: str) -> Dict[str, float]:
    """Evaluate response quality using RAGAS metrics"""
    try:
        return ragas_evaluator.evaluate_response_quality(question, answer, contexts, evaluator_llm, evaluator_llm_embedding)
    except Exception as e:
    ...
    if prompt := st.chat_input("Ask about NASA space missions..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching documents and generating response..."):
                ...
                context = ""
                contexts_list = []
                scores = [(1/(1+distance)) for distance in docs_result["distances"][0]]

                if docs_result and docs_result.get("documents"):
                    context = format_context(docs_result["documents"][0], docs_result["metadatas"][0], scores)
